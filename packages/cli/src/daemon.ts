...
import {
  ...
  handleEventsQuery,
  EpcisQueryError,
  type Publisher as EpcisPublisher,
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      const handled = await handleNodeUIRequest(
        ...
        chatAssistant,
        metricsCollector,
        authEnabled ? firstToken : undefined,
        memoryManager,
        llmSettings,
        telemetrySettings,
        resolveCorsOrigin(req, corsAllowed),
...
export function getOpenClawChannelTargets(
  config: DkgConfig,
...
async function handleRequest(
  ...
  if (req.method === "GET" && path === "/api/status") {
    ...
    return jsonResponse(res, 200, {
      ...
      hasOpenClawChannel:
        ...
        !!(
          config.openclawChannel?.bridgeUrl ||
          config.openclawChannel?.gatewayUrl
        ),
    ...
    for (const target of targets) {
      ...
      try {
        const transportRes = await fetch(
          target.streamUrl ?? target.inboundUrl,
          {
            method: "POST",
            headers: buildOpenClawChannelHeaders(target, bridgeAuthToken, {
              "Content-Type": "application/json",
  ...
  if (req.method === "GET" && path === "/api/openclaw-channel/health") {
    ...
    for (const target of targets) {
      ...
      try {
        ...
        const result: Record<string, unknown> & { ok: boolean } = {
          ok: healthRes.ok,
          ...parsed,
        };
        if (target.name === "bridge") {
  ...
  if (req.method === "GET" && path === "/api/integrations") {
    const [skills, paranets] = await Promise.all([
      agent.findSkills(),
    ...
    const adapters = [
      {
        ...
        name: "ElizaOS",
        enabled: true,
        description: "Connect to ElizaOS agents",
      },
      {
        id: "openclaw",
        name: "OpenClaw",
  ...
  if (req.method === "POST" && path === "/api/register-adapter") {
    ...
    if (typeof id !== "string" || id !== "openclaw") {
      return jsonResponse(res, 400, {
        error: `Unknown adapter id: ${String(id)}`,
      });
    }
