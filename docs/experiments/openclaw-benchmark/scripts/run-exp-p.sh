#!/usr/bin/env bash
#
# Experiment P (Parallel Control): 4 parallel Claude Code agents, NO DKG.
# Shared markdown coordination file only. Git worktrees for isolation.
#
# This is the missing cell in the comparison matrix:
#
#                  | Sequential  | 4 Parallel     |
#  ----------------+-------------+----------------+
#  No DKG          | Control ✓   | Exp-P (this)   |
#  DKG + SPARQL    | Exp-A ✓     | Exp-AB ✓       |
#
# Purpose: isolate whether Exp-AB's wall-time advantage comes from parallelism
# alone, or whether DKG actually helps when agents run in parallel.
#
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
EXPERIMENT_DIR="$(dirname "$SCRIPT_DIR")"
RESULTS_DIR="$EXPERIMENT_DIR/results/exp-p"
...
RUN_ID="${RUN_ID:-p-$(date +%Y%m%d-%H%M%S)}"

SHARED_DIR="$RESULTS_DIR/shared/$RUN_ID"
COORDINATION_MD="$SHARED_DIR/coordination.md"

mkdir -p "$RESULTS_DIR/round1" "$RESULTS_DIR/round2" "$SHARED_DIR"
chmod +x "$MD_LOG"

cat >"$COORDINATION_MD" <<EOF
...
echo "=== Experiment P: Parallel Control (4 agents, NO DKG, shared markdown) ==="
echo "  Run ID:   $RUN_ID"
...
setup_worktree() {
  ...
  mkdir -p "$(dirname "$wt")"
  git -C "$OPENCLAW_DIR" worktree add "$wt" -b "exp-p-${feature_id}" HEAD >/dev/null 2>&1 || true
  echo "$wt"
}
...
cleanup_worktrees() {
  ...
  if [[ -d "$base" ]]; then
    for wt in "$base"/*/; do
      ...
      git -C "$OPENCLAW_DIR" branch -D "exp-p-${name}" 2>/dev/null || true
    done
    rm -rf "$base"
  fi
}
...
run_feature_bg() {
  ...
  local agent="p-${feature_id}"

  local collab_prompt="COORDINATION PROTOCOL (shared markdown — NO DKG tools available)
runId: $RUN_ID
agentId: $agent
featureId: $feature_id
round: $round

You are one of 4 parallel agents, each building an independent feature in separate
git worktrees of the same OpenClaw codebase.

A shared coordination file exists at:
  $COORDINATION_MD

BEFORE starting work, READ this file to see what other agents are working on and
any patterns/interfaces they've documented.

As you work, LOG key decisions and discoveries by running:
  python \"$MD_LOG\" --file \"$COORDINATION_MD\" --run-id \"$RUN_ID\" --agent \"$agent\" --stream \"$feature_id\" --kind \"<kind>\" --text \"<note>\"

Use kind=plan when starting, kind=decision for key choices, kind=done when finished.
