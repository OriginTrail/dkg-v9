...
| `stateDir` | `<workspace>/.dkg-adapter` after setup | adapter runtime state directory; fresh setup stores chat-turn watermarks at `<workspace>/.dkg-adapter/chat-turn-watermarks.json` |
...
Environment override: `OPENCLAW_STATE_DIR` overrides the adapter state root at runtime. `~/.openclaw` remains the OpenClaw runtime home, not the adapter scratch directory.
