...
export async function subtractFinalizedExactQuads(params: {
  ...
  const confirmedRoots = await loadConfirmedRoots(params.store, params.graphManager, params.request.paranetId, params.validation.canonicalRoots);
  const authoritativePublic = await loadAuthoritativeQuadKeys(
    params.store,
    params.graphManager.dataGraphUri(params.request.paranetId),
    confirmedRoots,
  );
  const authoritativePrivate = await loadAuthoritativeQuadKeys(
    params.store,
    params.graphManager.privateGraphUri(params.request.paranetId),
    confirmedRoots,
  );
...
async function loadConfirmedRoots(
  ...
  const values = roots.map((root) => safeIri(root)).join(' ');
...
async function loadAuthoritativeQuadKeys(store: TripleStore, graph: string, confirmedRoots: Set<string>): Promise<Set<string>> {
  ...
  const values = [...confirmedRoots].map((root) => safeIri(root)).join(' ');
  const result = await store.query(
    `CONSTRUCT {
      ?s ?p ?o
    } WHERE {
