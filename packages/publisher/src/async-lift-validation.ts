...
export function validateLiftPublishPayload(input: LiftValidationInput): ValidatedLiftPublishPayload {
  ...
  const workspaceQuadCount = input.resolved.quads.length + (input.resolved.privateQuads?.length ?? 0);
  if (workspaceQuadCount === 0) {
    throw new Error('Lift validation requires at least one resolved workspace quad');
  }
...
function canonicalRootIri(request: LiftRequest, root: string): string {
  ...
  return `dkg:${slugPart(request.paranetId)}:${slugPart(request.namespace)}:${slugPart(request.scope)}/${rootName}-${rootHash}`;
}
